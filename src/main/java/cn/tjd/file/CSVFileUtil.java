...
public class CSVFileUtil {
    ...
    public CSVFileUtil(InputStream inputStream, String encode) throws Exception {
        this.ENCODE = encode;
     ...
     * @throws Exception
     */
    public List<String> readLineToList() throws Exception {
        String line = null;
        do {
            line = readLine();
        } while (line != null && line.replaceAll(",", "").trim().length() == 0);//如果当前行所有单元格是空的，则读取下一行
        if (line != null) {
            return fromCSVLinetoArray(line);
        }
        return null;
    }
     ...
     * @throws Exception
     */
    public List<LinkedHashMap<String, String>> readAll() throws Exception {
        List<LinkedHashMap<String, String>> data = new ArrayList<>();
        //读取表头
        List<String> titles = readLineToList();

        List<String> row = null;
        while ((row = readLineToList()) != null) {
            LinkedHashMap<String, String> rowMap = new LinkedHashMap<>();
            for (int i = 0; i < titles.size(); i++) {
                rowMap.put(titles.get(i).toUpperCase(), row.get(i));
            }
            data.add(rowMap);
        }
        return data;
